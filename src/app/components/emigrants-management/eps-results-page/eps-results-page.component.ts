...
@Component({
  selector: 'app-eps-results-page',
  templateUrl: './eps-results-page.component.html',
  styleUrls: ['./eps-results-page.component.css']
})
export class EpsResultsPageComponent implements OnInit {
  pageForm: FormGroup;
  pageData: any = null;
  isLoading = false;
  isEditing = false;
  ...
  loadPage(): void {
    this.isLoading = true;
    this.apiService.getEpsResultsPage().subscribe({
      next: (response: any) => {
  ...
  onSubmit(): void {
    ...
    this.isLoading = true;
    this.apiService.createOrUpdateEpsResultsPage(formData).subscribe({
      next: (response: any) => {
        if (response.success) {
