...
@Component({
  selector: 'app-oec-at-glance-page',
  templateUrl: './oec-at-glance-page.component.html',
  styleUrls: ['./oec-at-glance-page.component.css']
})
export class OecAtGlancePageComponent implements OnInit {
  pageForm: FormGroup;
  pageData: any = null;
  isLoading = false;
  isEditing = false;
  ...
  loadPage(): void {
    this.isLoading = true;
    this.apiService.getOecAtGlancePage().subscribe({
      next: (response: any) => {
  ...
  onSubmit(): void {
    ...
    this.isLoading = true;
    this.apiService.createOrUpdateOecAtGlancePage(formData).subscribe({
      next: (response: any) => {
        if (response.success) {
