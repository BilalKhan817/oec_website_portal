...
@Component({
  selector: 'app-md-message-page',
  templateUrl: './md-message-page.component.html',
  styleUrls: ['./md-message-page.component.css']
})
export class MdMessagePageComponent implements OnInit {
  pageForm: FormGroup;
  pageData: any = null;
  isLoading = false;
  isEditing = false;
  ...
  loadPage(): void {
    this.isLoading = true;
    this.apiService.getMdMessagePage().subscribe({
      next: (response: any) => {
  ...
  onSubmit(): void {
    ...
    this.isLoading = true;
    this.apiService.createOrUpdateMdMessagePage(formData).subscribe({
      next: (response: any) => {
        if (response.success) {
